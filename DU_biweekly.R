...
a <- 1:26
seq <- rep(a, each= 14)
BiWeek <- rep(c(seq, 26), 16)

rem.calcs$BiWeek <- BiWeek

Biweekly.summary <- rem.calcs %>%
  group_by(Wetland_ID, Water_year, BiWeek) %>%
  mutate(max.flow = max(VOL.IN)) %>%
  group_by(Wetland_ID, Water_year, BiWeek) %>%
  summarise(across(where(is.numeric), mean, na.rm = TRUE))

Biweekly.summary <- Biweekly.summary  %>%
...
ggplot(bw, aes(x = BiWeek, y = TP.rem.percent, group = 1)) +
  geom_line() +
  geom_point() +
...
ggplot(bw, aes(x = BiWeek, y = SRP.rem.percent, group = 1)) +
  geom_line() +
  geom_point() +
  facet_wrap(Wetland_ID~Water_year, scales = "free") +
  scale_x_continuous(" ", breaks = c(0, 10, 20, 30, 40, 50), 
